...
def main() -> int:
    ...
    manifest = Path(paths["meta_dir"]) / "brca_wsi_manifest.tsv"
    out_dir = Path(paths["raw_dir"]) / "wsi"
    ...
    runner_log = out_logs / f"11_download_wsi_{_ts(cfg)}.log"
    ...
    if args.dry_run:
        print("[DRY RUN] WSI download (ALL) preview")
